...
5. ~~**[CRITICAL]** Tenant scoping schema-only.~~ **FULLY CLOSED in p4-batch12 + p4-batch15** — `TenantContext` resolves the active tenant (override → X-Tenant-Id header → session → DEFAULT_TENANT_ID env → 1). `CookieRepository::performSave()` stamps `tenant_id` on insert. p4-batch15 added the read side: `CookieReadModelRepository` now accepts a `TenantContext` and scopes findById/findAll/findPaginated to the active tenant; `CookieReadModelProjection` stamps the same tenant id on the projected row so the read filter has something to match against. Multi-tenant deploys get isolation end-to-end through the read model; single-tenant deploys all converge on tenant_id=1 transparently.
