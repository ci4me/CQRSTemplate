...
3. ~~**[CRITICAL]** Read-model projection unwired.~~ **CLOSED across p2-batch1 + p4-batch9** — `Services::projectionRegistry()` builds the registry and registers `CookieReadModelProjection`; `subscribesTo()` returns all 5 lifecycle events. p4-batch9 added `CookieReadModelRepositoryInterface` (port) + `CookieReadModelRepository` (SQL-backed impl reading from `cookie_read_model`); `Services::cookieReadModelRepository()` wires it through. Query handlers will be migrated to depend on the new port as a single-PR per query in a follow-up; the breaking change is the handler constructor swap (cookieRepository → cookieReadModelRepository), so adoption is sequenced rather than bulk.
